...
template<typename_array_size_t start, typename_array_size_t end, typename array>
struct cut {
private:
	template<bool use, typename_array_size_t index, typename helper_array>
	struct cut_helper {
		using new_array = typename_array<>;
	};

	template<bool use, typename_array_size_t index, typename val, typename... other, template<typename, typename...> class templ>
	struct cut_helper<use, index, templ<val, other...>> {
		static constexpr typename_array_size_t next_index = index + 1;
		using new_array = typename cut_helper<((next_index >= start) && (next_index <= end)), next_index, templ<other...>>::new_array;
	};

	template<typename_array_size_t index, typename val, typename... other, template<typename, typename...> class templ>
